    ...
    <div x-data="{ activeTab: 'preliminary' }" label="Content tabs">
        ...
        <div class="mt-4">
            ...
            @foreach (collect($criteria)->unique('judge.id') as $judgeItem)
                ...
                <div class=" space-y-10" x-show="activeTab === '{{ $judgeItem['judge']['name'] }}'" x-cloak>
                    <x-printable-header />

                    <p class="text-center uppercase font-bold text-3xl"> {{ $judgeItem['judge']['name'] }}
                    </p>

                    @foreach ($uniqueCategories as $categoryItem)
                        @php
                            $categoryEntries = $judgeEntries->where(
                                'contest_category',
